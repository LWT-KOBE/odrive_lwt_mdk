...
volatile static uint32_t g_Initialized = 0;  // 初始化标志位
volatile static uint32_t g_Timestamp_us = 0; // 2的32次方/1000000 = 4294.967S后溢出
volatile static uint32_t g_SysClock = 0;     // 系统时钟
...
u8 DWT_Timer_Init(void)
{
    ...
    g_SysClock = rcc_clocks.HCLK_Frequency;            // HCLK是DWT计数器的时钟源
...
void DWT_Delay_us(uint32_t us)
{
    ...
    if(0 == g_Initialized) {
        DWT_Timer_Init();  // 如果发现没有初始化，就启动初始化
    }
    ticks = us * (DWT_Get_System_Clock_Freq() / 1000000u);  // 计算需要的节拍数
    told = DWT_Get_CNT();  // 获取刚进来时的计数值
    while(1){
        tnow = DWT_Get_CNT();  // 获取计数值
        if(tnow != told) {
            tcnt += tnow - told; // 计算节拍数
...
/**
 * @brief 计算us级的时间戳
 * @note 2的32次方/1000000 = 4294.967S后溢出
 * 
 */
static void DWT_Count_Timestamp_Microsecond(void)
{
    volatile uint32_t tnow = DWT_Get_CNT();  // 获取当前计数值
    volatile uint32_t mcu_fre = DWT_Get_System_Clock_Freq() / 1000000u;  // MCU每us的计数值，如果F4是168000000 / 1000 = 168000
    ...
    if(told != tnow) {  // 时间有变化
        tcnt += tnow - told;
        told = tnow;                // 更新时间
        tus = tcnt / mcu_fre;       // 看看经过多少us
        if(tus > 0) {
            g_Timestamp_us += tus;  // 积分us
            tcnt -= tus * mcu_fre;  // 减掉us
        }
    }
}
...
{
    DWT_Count_Timestamp_Microsecond(); // 更新时间戳
