	...
	switch(current_state_)
	{
		...
		case AXIS_STATE_ENCODER_OFFSET_CALIBRATION: {
			...
			if(status){
				...
				is_calibrated_ = 1;  //设置为已经校准
			}else{
				encoder_config.pre_calibrated = 0;  //设置为未校准
				is_calibrated_ = 0;  //设置为未校准
			}
		} break;
