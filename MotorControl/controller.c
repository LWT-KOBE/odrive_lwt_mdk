...
bool controller_update(void)
{
	...
	float *pos_wrap;
	float *vel_estimate;
	float *anticogging_pos_estimate;
	float *anticogging_vel_estimate;
	...
	anticogging_pos_estimate = &pos_estimate_;
	anticogging_vel_estimate = &vel_estimate_;
	
	if (!has_value(vel_estimate))return 0;
	
	if (anticogging.calib_anticogging)
	{
		// non-blocking
	...
	{
		vel_des = clamp(vel_des, -vel_lim, vel_lim);    //限制速度
	}
	...
	if(anticogging_valid_ && anticogging.anticogging_enabled)
	{
		float anticogging_pos = *anticogging_pos_estimate * COG_num;
		int cogindex = (int)anticogging_pos % COG_num;
		if(cogindex<0)cogindex=0;
	...
	if (ctrl_config.control_mode >= CONTROL_MODE_VELOCITY_CONTROL)
	{
		...
		torque += vel_integrator_torque_;                           //速度环I运算
	}
