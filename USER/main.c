...
//			if(strncmp((const char *)usb_recbuff, "M=", 2) == 0){
//				sscanf((const char *)usb_recbuff, "M=%f", &input_pos_); //设置当前位置，单位 turn
//				input_pos_updated_ = true;  //针对梯形轨迹模式，更新目标位置
//			}
			...
			// 设置输入模式
			// if(strstr((const char *)usb_recbuff, "Motor_Input_Mode=") != NULL){
			// 	sscanf((const char *)usb_recbuff, "Motor_Input_Mode=%d", &ctrl_config.input_mode); 
			// }
